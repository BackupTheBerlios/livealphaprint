...
/**
 * Data access layer for the Estimates table
 *
 ...
 *
 */
class Estimates extends SugarBean {
	...
	var $object_name = 'Estimates';
	var $module_dir = 'Estimates';
	var $new_schema = true;
	var $table_name = 'estimates';
	...
	function create_list_query($order_by, $where, $show_deleted = 0)
	{
		...
		$query = "SELECT users.user_name assigned_user_name, estimates.*";
...
function generate_email() {
	...
	function build_generic_where_clause ($the_query_string)
	{
		...
		array_push($where_clauses, "estimates.name LIKE '%$the_query_string%'");
	...
	function create_export_query(&$order_by, &$where)
    {
        ...
        $query .= " LEFT JOIN users
                   	ON estimates.assigned_user_id=users.id ";




        $where_auto = " estimates.deleted=0 ";
	...
	/*function components_estimate_check($id){
		...
		$query = ' SELECT status FROM estimates_components WHERE parent_id="'.$id.'" AND deleted=0 ';
		...
		$query = ' SELECT status FROM componentestimate WHERE estimate_id="'.$id.'" AND deleted=0 ';
		$result = $this->db->query($query,true,"");
		while($data = $this->db->fetchByAssoc($result)){
	...
	function estimate_estimate_check($id){
		$query = ' SELECT status FROM estimatecalc WHERE estimate_id="'.$id.'" AND deleted=0 ';
		$result = $this->db->query($query,true,"");
		$estimate = $this->db->fetchByAssoc($result);
		$estimate_count = $this->db->getRowCount($result);
		if ($estimate_count == 0){
			return true;
		}
		if ($estimate['status'] != 'uptodate'){
			return true;
		}
		return false; 
	}
	...
	function quote_check($id){
		$query = ' SELECT status FROM estimates WHERE id="'.$id.'" AND deleted=0 ';
		$result = $this->db->query($query,true,"");
		$estimate = $this->db->fetchByAssoc($result);
		...
		$estimate_estimate_check = $this->estimate_estimate_check($id);
		/*if ($this->status != "estimated"){
			return true;
		}*/
		if($estimate_estimate_check == true){
			return true;	
		}
	...
	function get_calc_record($id){
    	$query = " SELECT id FROM estimatecalc WHERE estimate_id='$id' AND deleted=0 ";
		$result = $this->db->query($query,true,"");
