...
if (isset($_REQUEST['parent_id'])){
	$focus->product_id = $_REQUEST['parent_id'];
}
...
$printrows = $focus->getPrintRows();

for ($i=0;$i<count($printrows);$i++) {
	    $fieldcount = count($printrows[$i]);
		$xtpl->assign("PRINTROWS",$focus->getPrintRow($printrows[$i],$i, true));
		$xtpl->parse("main.prints1");		
}

 
$xtpl->assign("count_print", count($printrows));  
$xtpl->assign("JAVASCRIPT", get_set_focus_js().get_validate_record_js(). calc_print_js() . $quicksearch_js);
...
$xtpl->assign("print_price", $focus->print_price);
