...
function getFormBody($prefix, $mod='', $formname=''){
	//Goodwill
	if(!ACLController::checkAccess('Estimates', 'edit', true)){
		return '';
	}
	//End Goodwill
	global $mod_strings;
...
function getForm($prefix, $mod='Estimates'){
	
	if(!ACLController::checkAccess('Estimates', 'edit', true, ''))return '';
...
$the_form .= <<<EOQ
			<form name="${prefix}EstimateSave" onSubmit="return check_form('${prefix}EstimateSave')" method="POST" action="index.php">
				<input type="hidden" name="${prefix}module" value="Estimates">
				<input type="hidden" name="${prefix}action" value="Save">
EOQ;
	$the_form .= $this->getFormBody($prefix, $mod, "${prefix}EstimateSave");
$the_form .= <<<EOQ
...
function handleSave($prefix,$redirect=true, $useRequired=false){
	...
	if (empty($_POST['record']) && empty($_POST['dup_checked'])) {
		$duplicateEstimates = $this->checkForDuplicates($prefix);
		if(isset($duplicateEstimates)){
			//$GLOBALS['log']->info("Duplicate Estimate:".$duplicateEstimates['id']);
			$this->handleRedirect($return_id, "Estimates");
			return null;
		}
	}
	//End Goodwill
