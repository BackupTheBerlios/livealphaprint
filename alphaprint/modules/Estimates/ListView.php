...
if(file_exists('custom/modules/Estimates/metadata/listviewdefs.php')){
    require_once('custom/modules/Estimates/metadata/listviewdefs.php');  
}else{
    require_once('modules/Estimates/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Estimates');
...
$seedEstimates = new Estimates(); // seed bean
$searchForm = new SearchForm('Estimates', $seedEstimates); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Estimates'][$col])) 
            $displayColumns[$col] = $listViewDefs['Estimates'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Estimates'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Estimates"); // builds the where clause from search field inputs
...
$lv->setup($seedEstimates, 'include/ListView/ListViewGeneric.tpl', $where, $params);
