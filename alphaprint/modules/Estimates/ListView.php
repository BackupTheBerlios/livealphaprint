...
$seedEstimate = new Estimate(); // seed bean
$searchForm = new SearchForm('Estimates', $seedEstimate); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Estimate'][$col])) 
            $displayColumns[$col] = $listViewDefs['Estimate'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Estimate'] as $col => $params) {
...
$lv->setup($seedEstimate, 'include/ListView/ListViewGeneric.tpl', $where, $params);
