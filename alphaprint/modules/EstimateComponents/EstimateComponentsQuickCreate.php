...
class EstimateComponentsQuickCreate extends QuickCreate {
    ...
    function process() {
        ...
        if($this->viaAJAX) { // override for ajax call
            $this->ss->assign('saveOnclick', "onclick='if(check_form(\"estimateComponentsQuickCreate\")) return SUGAR.subpanelUtils.inlineSave(this.form.id, \"estimatecomponents\"); else return false;'");
            $this->ss->assign('cancelOnclick', "onclick='return SUGAR.subpanelUtils.cancelCreate(\"subpanel_estimatecomponents\")';");
        }
        
        $this->ss->assign('viaAJAX', $this->viaAJAX);

        $this->javascript = new javascript();
        $this->javascript->setFormName('estimateComponentsQuickCreate');
		...
		$this->ss->assign("FORMAT_OPTIONS", get_select_options_with_id($app_list_strings['estimates_format_options'], $focus->format));
	    $this->ss->assign("RUN_FORMAT_OPTIONS", get_select_options_with_id($app_list_strings['estimates_format_options'], $focus->format));
	    $this->ss->assign("BLEED_FORMAT_OPTIONS", get_select_options_with_id($app_list_strings['estimates_format_options'], $focus->format));
...
/// SETUP PARENT POPUP
    ...
    $popup_request_data = array(
	'call_back_function' => 'set_return',
	'form_name' => 'estimateComponentsQuickCreate',
	'field_to_name_array' => array(
		'id' => 'paperid',
		'pname' => 'paper',
