...
if(file_exists('custom/modules/EstimateComponents/metadata/listviewdefs.php')){
    require_once('custom/modules/EstimateComponents/metadata/listviewdefs.php');  
}else{
    require_once('modules/EstimateComponents/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'EstimateComponents');
...
$seedEstimateComponents = new EstimateComponents(); // seed bean
$searchForm = new SearchForm('EstimateComponents', $seedEstimateComponents); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['EstimateComponents'][$col])) 
            $displayColumns[$col] = $listViewDefs['EstimateComponents'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['EstimateComponents'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "EstimateComponents"); // builds the where clause from search field inputs
...
$lv->setup($seedEstimateComponents, 'include/ListView/ListViewGeneric.tpl', $where, $params);
