...
EOQ;
    ...
    for ($i = 0; $i < sizeof($this->positions); $i ++) {
        ...
        $style = '';
        $explode[0] .= "<span id = 'slot$displayCount'  $class style='cursor: move$style'>";
        $explode[1] = "&nbsp;&nbsp;&nbsp;</span>".$explode[1];
       
        $slot[4] = preg_replace("'href[ ]*=[ ]*([\'])+[^\']*[\']+'is",'href=$1javascript:void(0)$1', $slot[4]);
        $slot[4] = preg_replace("'href[ ]*=[ ]*([\"])+[^\"]*[\"]+'is",'href=$1javascript:void(0)$1', $slot[4]);
        $slot[4] = str_replace(array("<a ", '</a '),array('<span', '</span>'), $slot[4]);
