...
// $Id: QuickCreate.tpl,v 1.1 2007/01/22 13:15:08 peshev Exp $
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
	...
	<td align="left" style="padding-bottom: 2px;">
		<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="button" type="submit" name="button" {$saveOnclick|default:"onclick=\"return check_form('QuotesQuickCreate');\""} value="  {$APP.LBL_SAVE_BUTTON_LABEL}  " >
		<input title="{$APP.LBL_CANCEL_BUTTON_TITLE}" accessKey="{$APP.LBL_CANCEL_BUTTON_KEY}" class="button" type="submit" name="button" {$cancelOnclick|default:"onclick=\"this.form.action.value='$RETURN_ACTION'; this.form.module.value='$RETURN_MODULE'; this.form.record.value='$RETURN_ID'\""} value="  {$APP.LBL_CANCEL_BUTTON_LABEL}  ">
		<input title="{$APP.LBL_FULL_FORM_BUTTON_TITLE}" accessKey="{$APP.LBL_FULL_FORM_BUTTON_KEY}" class="button" type="submit" name="button" onclick="this.form.to_pdf.value='0';this.form.action.value='EditView'; this.form.module.value='Quotes';" value="  {$APP.LBL_FULL_FORM_BUTTON_LABEL}  "></td>
...
<td>
    ...
    <tr>
    <td valign="top" class="dataLabel" rowspan="2"><slot>{$MOD.LBL_QUOTENUM}<span class="required">{$APP.LBL_REQUIRED_SYMBOL}</span></slot></td>
