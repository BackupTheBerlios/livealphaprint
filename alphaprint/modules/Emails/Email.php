...
class Email extends SugarBean {
	...
	function fillPrimaryParentFields() {
		...
		$tables = array('accounts', 'contacts', 'leads', 'users');
		$ret = array();
	...
	function getForwardHeader() {
		...
		$ret .= "&gt; {$mod_strings['LBL_FROM']} {$this->from_name}<br />";
		$ret .= "&gt; {$mod_strings['LBL_DATE_SENT']} {$this->date_sent}<br />";
		$ret .= "&gt; {$mod_strings['LBL_TO']} {$this->to_addrs}<br />";
		$ret .= "&gt; {$mod_strings['LBL_SUBJECT']} {$this->name}<br />";
		$ret .= "&gt; <br />";
		$ret .= "&gt; <br />"; 
		return $ret;
	}
	...
	function quoteHtmlEmail() {
		...
		$text = br2nl($text);
		// strip bad tags
		$text = strip_tags($text, "<p><span><div><table><tr><td><th>");
