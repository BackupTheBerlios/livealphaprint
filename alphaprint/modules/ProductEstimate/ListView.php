...
if(file_exists('custom/modules/ProductEstimate/metadata/listviewdefs.php')){
    require_once('custom/modules/ProductEstimate/metadata/listviewdefs.php');  
}else{
    require_once('modules/ProductEstimate/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'ProductEstimate');
...
$seedProductEstimate = new ProductEstimate(); // seed bean
$searchForm = new SearchForm('ProductEstimate', $seedProductEstimate); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['ProductEstimate'][$col])) 
            $displayColumns[$col] = $listViewDefs['ProductEstimate'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['ProductEstimate'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "ProductEstimate"); // builds the where clause from search field inputs
...
$lv->setup($seedProductEstimate, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('ProductEstimate')));
$str = "<script>
