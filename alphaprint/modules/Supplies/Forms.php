...
function get_product_js () {
    ...
    function gennumber(form) {
        ...
        var numb = pref + suf;
        form.supplynum.value = numb;
        form.name.value = numb;
    }
    ...
    function totalp(v){
       ...
       for (var index=1;index<=count;index++){
         var price = form["price_" + index].value;
         p = price * 1;
         tp += p;
       }
       form["total_price"].value = tp.toFixed(2);
    }
       
    
    function calcprice(v) {
       var name = v.name;
       var idx = name.substring(name.lastIndexOf("_")+1);
       var form = v.form;
       
       var singlep = form["singlep_"+idx].value;
       var unit = form["unit_" + idx].value;
       
       if(isNaN(singlep)) {
            alert('$err_invalidnumber');
            v.value="";
            v.focus();
            return;
        }
       ...
       var price = (singlep * unit).toFixed(2);
       form["price_" + idx].value = price;
    }
	...
	{ 
		 ...
		 sHTML = sHTML.replace('materialindex=0','index=' + count);
         sHTML = sHTML.replace('paperindex=0','index=' + count);
         sHTML = sHTML.replace('number','number_' + count );
		 sHTML = sHTML.replace('numvalue', count );
		 sHTML = sHTML.replace('paperid','paperid_' + count );
         sHTML = sHTML.replace('materialid','materialid_' + count );
		 sHTML = sHTML.replace('materialname','materialname_' + count);
         sHTML = sHTML.replace('measure','measure_' + count ); 
         sHTML = sHTML.replace('unit','unit_' + count ); 
         sHTML = sHTML.replace('singlep','singlep_' + count ); 
	...
	{ 
		 ...
		 {
		 	alert('$err_one_material_required');
		 	return;
		 }
	...
	function checkDup(materialid, idx)
	{
		for(var i = 0; i < count; i++)
		{
			if(idx == i) continue;
			
			var elem = getElementsByName("materialid_" + i);
			if(elem == null) break;
				
			if(elem[0].value==materialid)
			{
	...
	function check_material(form)
	{
		...
		for(var i = 1; i <= count ; i++)
		{
			...
			var materialname = form.elements['materialname_' + i];
		...
		if(!hasMaterial)alert('$err_one_material_required');
		return hasMaterial;
	}
	//-->
	</script>
