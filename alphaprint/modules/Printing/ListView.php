...
if(file_exists('custom/modules/Printing/metadata/listviewdefs.php')){
    require_once('custom/modules/Printing/metadata/listviewdefs.php');  
}else{
    require_once('modules/Printing/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Printing');
...
$seedPrinting = new Printing(); // seed bean
$searchForm = new SearchForm('Printing', $seedPrinting); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Printing'][$col])) 
            $displayColumns[$col] = $listViewDefs['Printing'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Printing'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Printing"); // builds the where clause from search field inputs
...
$lv->setup($seedPrinting, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Printing')));
$str = "<script>
