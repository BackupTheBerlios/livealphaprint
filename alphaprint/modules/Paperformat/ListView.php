...
if(file_exists('custom/modules/Paperformat/metadata/listviewdefs.php')){
    require_once('custom/modules/Paperformat/metadata/listviewdefs.php');  
}else{
    require_once('modules/Paperformat/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Paperformat');
...
$seedPaperformat = new Paperformat(); // seed bean
$searchForm = new SearchForm('Paperformat', $seedPaperformat); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Paperformat'][$col])) 
            $displayColumns[$col] = $listViewDefs['Paperformat'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Paperformat'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Paperformat"); // builds the where clause from search field inputs
...
$lv->setup($seedPaperformat, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Paperformat')));
$str = "<script>
