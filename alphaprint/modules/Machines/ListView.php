...
if(file_exists('custom/modules/Machines/metadata/listviewdefs.php')){
	require_once('custom/modules/Machines/metadata/listviewdefs.php');	
}else{
	require_once('modules/Machines/metadata/listviewdefs.php');
}
...
$seedMachine = new Machine(); // seed bean
$searchForm = new SearchForm('Machines', $seedMachine); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Machines'][$col])) 
            $displayColumns[$col] = $listViewDefs['Machines'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Machines'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Machines"); // builds the where clause from search field inputs
...
$lv->setup($seedMachine, 'include/ListView/ListViewGeneric.tpl', $where, $params);
