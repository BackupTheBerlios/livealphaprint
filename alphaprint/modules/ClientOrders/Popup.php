...
//{CLIENTORDER.ID}", "{CLIENTORDER.NAME}", "{CLIENTORDER.NUMBER}", "{CLIENTORDER.UNITPRICE}")
/*
sHTML = sHTML.replace('cost_price','cost_price_' + count);
	 ...
	 sHTML = sHTML.replace('clientorder_name','clientorder_name_' + count);
	 sHTML = sHTML.replace('clientorder_id','clientorder_id_' + count);
	 sHTML = sHTML.replace('clientorder_num','clientorder_num_' + count);
*/
if($_REQUEST['form'] == 'EditView')
{
	...
	$the_javascript .= "	window.opener.document.EditView.clientorderid_$index.value = id;\n";
	$the_javascript .= "	window.opener.document.EditView.clientordername_$index.value = name;\n";
	$the_javascript .= "	window.opener.document.EditView.clientordernum_$index.value = number;\n";
	...
	$the_javascript .= "	window.opener.document.EditView.clientorder_id_$index.value = ''; \n";
	$the_javascript .= "	window.opener.document.EditView.clientorder_name_$index.value = ''; \n";
	$the_javascript .= "	window.opener.document.EditView.clientorder_num_$index.value = ''; \n";
...
require_once('modules/ClientOrders/ClientorderFormBase.php');
$formBase = new ClientorderFormBase();
...
$createContact = <<<EOQ
<input class='button' type='button' name='showAdd' value='{$mod_strings['LBL_ADD_CLIENTORDER']}' onClick='toggleDisplay("addform");'>
EOQ;
$form->assign("CREATECLIENTORDER", $createContact);

$form->assign("ADDFORMHEADER", get_form_header($mod_strings['LBL_ADD_CLIENTORDER'], $formSave, false));
...
$ListView->setQuery($where, "", "number, name", "CLIENTORDER");
$ListView->setModStrings($mod_strings);
$ListView->process_for_popups = true;
$ListView->processListView($seed_object, "main", "CLIENTORDER");
