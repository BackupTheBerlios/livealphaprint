...
if(file_exists('custom/modules/ClientOrders/metadata/listviewdefs.php')){
    require_once('custom/modules/ClientOrders/metadata/listviewdefs.php');  
}else{
    require_once('modules/ClientOrders/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'ClientOrders');
...
$seedClientOrders = new ClientOrders(); // seed bean
$searchForm = new SearchForm('ClientOrders', $seedClientOrders); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['ClientOrders'][$col])) 
            $displayColumns[$col] = $listViewDefs['ClientOrders'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['ClientOrders'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "ClientOrders"); // builds the where clause from search field inputs
...
$lv->setup($seedClientOrders, 'include/ListView/ListViewGeneric.tpl', $where, $params);
