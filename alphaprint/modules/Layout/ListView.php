...
if(file_exists('custom/modules/Layout/metadata/listviewdefs.php')){
    require_once('custom/modules/Layout/metadata/listviewdefs.php');  
}else{
    require_once('modules/Layout/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Layout');
...
$seedLayout = new Layout(); // seed bean
$searchForm = new SearchForm('Layout', $seedLayout); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Layout'][$col])) 
            $displayColumns[$col] = $listViewDefs['Layout'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Layout'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Layout"); // builds the where clause from search field inputs
...
$lv->setup($seedLayout, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Layout')));
$str = "<script>
