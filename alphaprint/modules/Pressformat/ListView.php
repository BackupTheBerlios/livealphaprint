...
if(file_exists('custom/modules/Pressformat/metadata/listviewdefs.php')){
    require_once('custom/modules/Pressformat/metadata/listviewdefs.php');  
}else{
    require_once('modules/Pressformat/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Pressformat');
...
$seedPressformat = new Pressformat(); // seed bean
$searchForm = new SearchForm('Pressformat', $seedPressformat); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Pressformat'][$col])) 
            $displayColumns[$col] = $listViewDefs['Pressformat'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Pressformat'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Pressformat"); // builds the where clause from search field inputs
...
$lv->setup($seedPressformat, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Pressformat')));
$str = "<script>
