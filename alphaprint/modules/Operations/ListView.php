...
if(file_exists('custom/modules/Operations/metadata/listviewdefs.php')){
    require_once('custom/modules/Operations/metadata/listviewdefs.php');  
}else{
    require_once('modules/Operations/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Operations');
...
$seedOperation = new Operation(); // seed bean
$searchForm = new SearchForm('Operations', $seedOperation); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Operation'][$col])) 
            $displayColumns[$col] = $listViewDefs['Operation'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Operation'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Operations"); // builds the where clause from search field inputs
...
$lv->setup($seedOperation, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Operations')));
$str = "<script>
