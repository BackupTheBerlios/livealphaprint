...
class InboundEmail extends SugarBean {
	...
	function buildBreadCrumbs($parts, $subtype, $breadcrumb = '0') {
		...
		foreach($parts as $k => $part) {
			...
			if($part->type == 1 && (strtoupper($part->subtype) == 'ALTERNATIVE' || strtoupper($part->subtype) == 'MIXED')) {
	...
	function importDupeCheck($message_id, $msgNo, $header, $textHeader) {
		...
		$query = 'SELECT count(emails.id) AS c FROM emails WHERE emails.message_id = \''.$this->compoundMessageId.'\' AND deleted = 0';
