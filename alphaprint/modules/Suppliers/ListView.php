...
if(file_exists('custom/modules/Suppliers/metadata/listviewdefs.php')){
	require_once('custom/modules/Suppliers/metadata/listviewdefs.php');	
}else{
	require_once('modules/Suppliers/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Suppliers');
...
$seedSupplier = new Supplier();
$searchForm = new SearchForm('Suppliers', $seedSupplier);
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Suppliers'][$col])) 
            $displayColumns[$col] = $listViewDefs['Suppliers'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) {
    ...
    foreach($listViewDefs['Suppliers'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Suppliers");
...
$lv->setup($seedSupplier, 'include/ListView/ListViewGeneric.tpl', $where, $params);
