 ...
 * Description:  base form for supplier
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.
 * All Rights Reserved.
 * Contributor(s): ______________________________________..
 ********************************************************************************/
 
class SupplierFormBase{
...
function checkForDuplicates($prefix){
	...
	$focus = new Supplier();
	$query = '';
	$baseQuery = 'select id, name, website, billing_address_city  from suppliers where deleted!=1 and ';
...
$the_form .= <<<EOQ
		<form name="${prefix}SupplierSave" onSubmit="return check_form('${prefix}SupplierSave');" method="POST" action="index.php">
			<input type="hidden" name="${prefix}module" value="Suppliers">
			<input type="hidden" name="${prefix}action" value="Save">
EOQ;
$the_form .= $this->getFormBody($prefix, $mod, $prefix."SupplierSave");
$the_form .= <<<EOQ
...
$user_id = $current_user->id;
	...
	$form = <<<EOQ
		...
		$lbl_supplier_name&nbsp;<span class="required">$lbl_required_symbol</span><br>
...
EOQ;
	...
	if (method_exists($contact, 'convertCustomFieldsForm')) $contact->convertCustomFieldsForm($form, $tempSupplier, $prefix);
	unset($tempSupplier);
$form .= <<<EOQ
		</TABLE>
EOQ;
