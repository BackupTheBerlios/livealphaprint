...
function checkForDuplicates($prefix){
	...
	$focus = new Product();
	$query = '';
	$baseQuery = 'select id, pnum from products where deleted!=1 and ';
...
EOQ;
	...
	$javascript->setSugarBean(new Products());
...
function getForm($prefix, $mod='Products'){
	
	if(!ACLController::checkAccess('Products', 'edit', true, ''))return '';
...
$the_form .= <<<EOQ
			<form name="${prefix}ProductSave" onSubmit="return check_form('${prefix}ProductSave')" method="POST" action="index.php">
				<input type="hidden" name="${prefix}module" value="Products">
				<input type="hidden" name="${prefix}action" value="Save">
EOQ;
	$the_form .= $this->getFormBody($prefix, $mod, "${prefix}ProductSave");
$the_form .= <<<EOQ
...
function handleSave($prefix,$redirect=true, $useRequired=false){
	...
	if (empty($_POST['record']) && empty($_POST['dup_checked'])) {
		$duplicateProducts = $this->checkForDuplicates($prefix);
		if(isset($duplicateProducts)){
			//$GLOBALS['log']->info("Duplicate Product:".$duplicateProducts['id']);
			$this->handleRedirect($return_id, "Products");
			return null;
		}
	}
	//End Goodwill
