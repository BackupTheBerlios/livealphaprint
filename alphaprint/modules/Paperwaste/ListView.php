...
if(file_exists('custom/modules/Paperwaste/metadata/listviewdefs.php')){
    require_once('custom/modules/Paperwaste/metadata/listviewdefs.php');  
}else{
    require_once('modules/Paperwaste/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Paperwaste');
...
$seedPaperwaste = new Paperwaste(); // seed bean
$searchForm = new SearchForm('Paperwaste', $seedPaperwaste); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Paperwaste'][$col])) 
            $displayColumns[$col] = $listViewDefs['Paperwaste'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Paperwaste'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Paperwaste"); // builds the where clause from search field inputs
...
$lv->setup($seedPaperwaste, 'include/ListView/ListViewGeneric.tpl', $where, $params);
