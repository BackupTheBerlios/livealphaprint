...
/**
 * Data access layer for the paperwaste table
 *
 ...
 *
 */
class Paperwaste extends SugarBean {
	...
	var $object_name = 'Paperwaste';
	var $module_dir = 'Paperwaste';
	var $new_schema = true;
	var $table_name = 'paperwaste';
	...
	function create_list_query($order_by, $where, $show_deleted = 0)
	{
		...
		$query = "SELECT users.user_name assigned_user_name, paperwaste.*";
		...
		$query .= " FROM paperwaste ";




          $query .= "LEFT JOIN users ON paperwaste.assigned_user_id=users.id ";
          //$query .= "LEFT JOIN pricelist_relation ON paperwaste.id=pricelist_relation.pricelist_id ";
...
//die($query);
	...
	function getPaperwastelineRows() {
		$return_array = array();
		if($this->id != "") {
			$paperwasteline = new Paperwasteline();
			$return_array = $paperwasteline->get_full_list("id","paperwaste_id='".$this->id."'");
		}
		return $return_array;
	}

	function getPaperwastelineRow($pricelistrow,$index,$is_editview=false) {
		global $mod_strings;
		global $app_strings;
		...
		{                                             // Listview
            ...
            $tablerow = $tablerow.'				<TD class=tabDetailViewDF width=10%>'.$pricelistrow->base_waste.'</TD>';
            $tablerow = $tablerow.'				<TD class=tabDetailViewDF width=10%>'.$pricelistrow->step_waste.'</TD>';
		...
		{                                         //Editview
            ...
            $tablerow = $tablerow.'             <TD class=dataField width=30%><input type=text size=7 value="'.$pricelistrow->base_waste.'" id=base_waste_'.$count.' name=base_waste_'.$count.'></TD>';
            $tablerow = $tablerow.'             <TD class=dataField width=30%><input type=text size=7 value="'.$pricelistrow->step_waste.'" id=step_waste_'.$count.' name=step_waste_'.$count.'></TD>';			
	...
	function build_generic_where_clause ($the_query_string)
	{
		...
		array_push($where_clauses, "paperwaste.name LIKE '%$the_query_string%'");
	...
	function create_export_query(&$order_by, &$where)
    {
        ...
        $query .= " LEFT JOIN users
                   	ON paperwaste.assigned_user_id=users.id ";




        $where_auto = " paperwaste.deleted=0 ";
