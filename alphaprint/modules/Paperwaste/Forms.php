...
{
	if(!ACLController::checkAccess('Paperwaste', 'edit', true))return '';
	global $app_strings;
	global $mod_strings;
	global $currentModule;
	global $current_user;

	$the_form = get_left_form_header($mod_strings['LBL_QUICK_NEW_PAPERWASTE']);
	$form = new XTemplate ('modules/Paperwaste/Forms.html');
...
function get_pricelist_js () {
    ...
    { 
         ...
         sHTML = sHTML.replace('base_waste','base_waste_' + count ); 
         sHTML = sHTML.replace('step_waste','step_waste_' + count );
 		 
		 sHTML = sHTML.replace('impressionsnumberid','impressions_number_' + count );
		 sHTML = sHTML.replace('basewasteid','base_waste_' + count );
		 sHTML = sHTML.replace('stepwasteid','step_waste_' + count );
	...
	function set_waste_type(){
		...
		if (type == "Press"){
			...
			document.getElementById("setup_waste_per_plate").value = "";
			document.getElementById("setup_waste_per_plate").setAttribute('readOnly','readonly');
			document.getElementById("operation_button").removeAttribute('disabled');
			document.getElementById("operation_name").removeAttribute('readOnly');
		}
