...
if(file_exists('custom/modules/Pressmachine/metadata/listviewdefs.php')){
    require_once('custom/modules/Pressmachine/metadata/listviewdefs.php');  
}else{
    require_once('modules/Pressmachine/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Pressmachine');
...
$seedPressmachine = new Pressmachine(); // seed bean
$searchForm = new SearchForm('Pressmachine', $seedPressmachine); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Pressmachine'][$col])) 
            $displayColumns[$col] = $listViewDefs['Pressmachine'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Pressmachine'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Pressmachine"); // builds the where clause from search field inputs
...
$lv->setup($seedPressmachine, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Pressmachine')));
$str = "<script>
