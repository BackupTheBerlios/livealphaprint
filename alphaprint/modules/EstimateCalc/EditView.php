...
$xtpl=new XTemplate ('modules/EstimateCalc/EditView.html');
...
if (!is_null($focus->name) && !empty($focus->name))
{
	...
	$xtpl->assign("NAME", $mod_strings['LBL_EST_NAME_PREFIX']."-".$estimate_name);
}

if (!is_null($focus->estimate_name) && !empty($focus->estimate_name))
{
	$xtpl->assign("estimate_name", $focus->estimate_name);
}
else{
	$xtpl->assign("estimate_name", $estimate_name);
}

if (!is_null($focus->estimate_id) && !empty($focus->estimate_id))
{
	$xtpl->assign("estimate_id", $focus->estimate_id);
}
else{
	$xtpl->assign("estimate_id", $estimate_id);
}
