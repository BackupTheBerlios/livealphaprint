...
if(file_exists('custom/modules/EstimateCalc/metadata/listviewdefs.php')){
    require_once('custom/modules/EstimateCalc/metadata/listviewdefs.php');  
}else{
    require_once('modules/EstimateCalc/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'EstimateCalc');
...
$seedEstimateCalc = new EstimateCalc(); // seed bean
$searchForm = new SearchForm('EstimateCalc', $seedEstimateCalc); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['EstimateCalc'][$col])) 
            $displayColumns[$col] = $listViewDefs['EstimateCalc'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['EstimateCalc'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "EstimateCalc"); // builds the where clause from search field inputs
...
$lv->setup($seedEstimateCalc, 'include/ListView/ListViewGeneric.tpl', $where, $params);
