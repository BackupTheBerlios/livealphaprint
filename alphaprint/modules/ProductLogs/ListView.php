...
if(file_exists('custom/modules/ProductLogs/metadata/listviewdefs.php')){
    require_once('custom/modules/ProductLogs/metadata/listviewdefs.php');  
}else{
    require_once('modules/ProductLogs/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'ProductLogs');
...
$seedProductLog = new ProductLog(); // seed bean
$searchForm = new SearchForm('ProductLogs', $seedProductLog); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['ProductLog'][$col])) 
            $displayColumns[$col] = $listViewDefs['ProductLog'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['ProductLog'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "ProductLogs"); // builds the where clause from search field inputs
...
$lv->setup($seedProductLog, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('ProductLogs')));
$str = "<script>
