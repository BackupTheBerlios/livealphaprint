...
if(file_exists('custom/modules/ComponentEstimate/metadata/listviewdefs.php')){
    require_once('custom/modules/ComponentEstimate/metadata/listviewdefs.php');  
}else{
    require_once('modules/ComponentEstimate/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'ComponentEstimate');
...
$seedComponentEstimate = new ComponentEstimate(); // seed bean
$searchForm = new SearchForm('ComponentEstimate', $seedComponentEstimate); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['ComponentEstimate'][$col])) 
            $displayColumns[$col] = $listViewDefs['ComponentEstimate'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['ComponentEstimate'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "ComponentEstimate"); // builds the where clause from search field inputs
...
$lv->setup($seedComponentEstimate, 'include/ListView/ListViewGeneric.tpl', $where, $params);
