...
if(file_exists('custom/modules/ClientRequest/metadata/listviewdefs.php')){
    require_once('custom/modules/ClientRequest/metadata/listviewdefs.php');  
}else{
    require_once('modules/ClientRequest/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'ClientRequest');
...
$seedClientRequest = new ClientRequest(); // seed bean
$searchForm = new SearchForm('ClientRequest', $seedClientRequest); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['ClientRequest'][$col])) 
            $displayColumns[$col] = $listViewDefs['ClientRequest'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['ClientRequest'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "ClientRequest"); // builds the where clause from search field inputs
...
$lv->setup($seedClientRequest, 'include/ListView/ListViewGeneric.tpl', $where, $params);
