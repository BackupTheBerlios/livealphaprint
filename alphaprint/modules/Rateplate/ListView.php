...
if(file_exists('custom/modules/Rateplate/metadata/listviewdefs.php')){
    require_once('custom/modules/Rateplate/metadata/listviewdefs.php');  
}else{
    require_once('modules/Rateplate/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Rateplate');
...
$seedRateplate = new Rateplate(); // seed bean
$searchForm = new SearchForm('Rateplate', $seedRateplate); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Rateplate'][$col])) 
            $displayColumns[$col] = $listViewDefs['Rateplate'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Rateplate'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Rateplate"); // builds the where clause from search field inputs
...
$lv->setup($seedRateplate, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Rateplate')));
$str = "<script>
