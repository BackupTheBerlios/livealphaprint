...
// $Id: QuickCreate.tpl,v 1.2 2007/01/30 16:11:35 ivan_garbev Exp $

*}


<form name="purchaseordersQuickCreate" id="purchaseordersQuickCreate" method="POST" action="index.php">
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
	...
	<td align="left" style="padding-bottom: 2px;">
		<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="button" type="submit" name="button" {$saveOnclick|default:"onclick=\"return check_form('PurchaseOrdersQuickCreate');\""} value="  {$APP.LBL_SAVE_BUTTON_LABEL}  " >
		<input title="{$APP.LBL_CANCEL_BUTTON_TITLE}" accessKey="{$APP.LBL_CANCEL_BUTTON_KEY}" class="button" type="submit" name="button" {$cancelOnclick|default:"onclick=\"this.form.action.value='$RETURN_ACTION'; this.form.module.value='$RETURN_MODULE'; this.form.record.value='$RETURN_ID'\""} value="  {$APP.LBL_CANCEL_BUTTON_LABEL}  ">
		<input title="{$APP.LBL_FULL_FORM_BUTTON_TITLE}" accessKey="{$APP.LBL_FULL_FORM_BUTTON_KEY}" class="button" type="submit" name="button" onclick="this.form.to_pdf.value='0';this.form.action.value='EditView'; this.form.module.value='PurchaseOrders';" value="  {$APP.LBL_FULL_FORM_BUTTON_LABEL}  "></td>
...
<td>
	...
	<td class="dataLabel" width="15%"><slot>{$MOD.LBL_STAGE} <span class="required">{$APP.LBL_REQUIRED_SYMBOL}</span></slot></td>
	<td width="35%"><slot><select tabindex='2' name='status'>{$STAGE_OPTIONS}</select></slot></td>
	</tr>
	
    <tr>
    <td valign="top" class="dataLabel" rowspan="2"><slot>{$MOD.LBL_PURCHASEORDERNUM}<span class="required">{$APP.LBL_REQUIRED_SYMBOL}</span></slot></td>
	<td rowspan="2"><slot><input name='purchaseordernum' type="text" tabindex='1' size='25' maxlength='50'  value=""></slot></td>
	<td class="dataLabel"><slot>{$MOD.LBL_ACCOUNT_NAME}</slot></td>
