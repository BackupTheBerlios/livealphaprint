...
if(file_exists('custom/modules/Prices/metadata/listviewdefs.php')){
    require_once('custom/modules/Prices/metadata/listviewdefs.php');  
}else{
    require_once('modules/Prices/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Prices');
...
$seedPrice = new Price(); // seed bean
$searchForm = new SearchForm('Prices', $seedPrice); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Price'][$col])) 
            $displayColumns[$col] = $listViewDefs['Prices'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Prices'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Prices"); // builds the where clause from search field inputs
...
$lv->setup($seedPrice, 'include/ListView/ListViewGeneric.tpl', $where, $params);
