...
$currencies = array();
function updateAmountBySymbol($id, $symbol, $price){
	...
	if(isset($id) && !empty($id)){
			...
			$query = "update opportunities set price='$price', currency_id='$currencyID', price_usdollar='$dollars' where id='$id';";
			$db->query($query);
			}
}
}
...
while($row = $db->fetchByAssoc($result)){
		...
		if($row['price'] == NULL){
			...
			print '<tr><td>'.$mod_strings['UPDATE_VERIFY_FAIL'] . '</td><td><a href="index.php?module=Opportunities&action=DetailView&record='.$row['id']. '" target="_blank">'.$row['name']. '</a></tr><tr><td></td><td>' . $mod_strings[ 'UPDATE_VERIFY_CURAMOUNT'] . '&nbsp;' . $row['price']. '</td></tr><tr><td></td><td><b>'. $mod_strings['UPDATE_VERIFY_FIX']. '</b><br>'. $mod_strings['UPDATE_VERIFY_NEWAMOUNT']. '&nbsp;' . $price . '<br>' . $mod_strings['UPDATE_VERIFY_NEWCURRENCY'] . $symbol . '</td></tr>';
			}
...
function doUpdateDollarAmounts($closed = false){
	global $db, $currency, $currencies, $mod_strings;
	$query = "select name, id,currency_id, price from prices";
	if(!$closed){
		$query.= " where prices.sales_stage <> 'Closed Won' AND prices.sales_stage <> 'Closed Lost';";
	}
	$result = $db->query($query);
...
while($row = $db->fetchByAssoc($result)){
			...
			updateAmountByID($row['id'], $row['currency_id'] ,$row['price']);
			$count++;
			}else{
...
function doRestoreAmounts(){
	global $db, $currency, $currencies, $mod_strings;
	$query = 'select id, price_backup from prices;';
	$result = $db->query($query);
	$count = 0;
