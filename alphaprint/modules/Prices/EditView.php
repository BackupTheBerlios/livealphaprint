...
require_once('modules/Prices/Forms.php');
...
$GLOBALS['log']->info("Price detail view");

$xtpl=new XTemplate ('modules/Prices/EditView.html');
...
$xtpl->assign('encoded_suppliers_popup_request_data', $json->encode($popup_request_data));
...
//Isvalid
$current_date = $timedate->to_display_date(gmdate('Y-m-d H:i:s'));
if (empty($focus->due_date)){
	$xtpl->assign('isvalid', $focus->isvalid);
}
elseif($current_date > $focus->due_date){
	
	$focus->update_isvalid();
	$focus->isvalid = 'Not Valid';
	$xtpl->assign('isvalid', $focus->isvalid);
}
$xtpl->assign('ISVALID_OPTIONS',get_select_options_with_id($app_list_strings['isvalid_options'], $focus->isvalid));

$xtpl->assign("PRICE", $focus->price);

//Material Link
if(isset($focus->paper_id)){
$xtpl->assign("PAPER_ID", $focus->paper_id);	
}

if(isset($focus->material_id)){
$xtpl->assign("MATERIAL_ID", $focus->material_id);	
}

if(isset($focus->supplier_id)){
...
$json = getJSONobj();
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Prices')));
$str = "<script>
