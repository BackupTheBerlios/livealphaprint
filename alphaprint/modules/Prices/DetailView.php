...
if (isset($_REQUEST['offset']) or isset($_REQUEST['record'])) {
	$result = $detailView->processSugarBean("PRICE", $focus, $offset);
	if($result == null) {
...
$xtpl = new XTemplate('modules/Prices/DetailView.html');
...
//Supplier
if (isset($focus->supplier_id) && !empty($focus->supplier_id)){
...
if ($focus->paper_id){
	$xtpl->assign('MODULE', "Paper");
	$xtpl->assign('MTR_ID', $focus->paper_id);
}
if ($focus->material_id){
	$xtpl->assign('MODULE', "Materials");
	$xtpl->assign('MTR_ID', $focus->material_id);
}
...
$detailView->processListNavigation($xtpl, "PRICE", $offset);
...
$subpanel = new SubPanelTiles($focus, 'Price');
...
$json = getJSONobj();
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Price')));
$str = "<script>
