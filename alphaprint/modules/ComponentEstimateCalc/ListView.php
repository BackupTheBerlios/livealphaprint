...
if(file_exists('custom/modules/ComponentEstimateCalc/metadata/listviewdefs.php')){
    require_once('custom/modules/ComponentEstimateCalc/metadata/listviewdefs.php');  
}else{
    require_once('modules/ComponentEstimateCalc/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'ComponentEstimateCalc');
...
$seedComponentEstimateCalc = new ComponentEstimateCalc(); // seed bean
$searchForm = new SearchForm('ComponentEstimateCalc', $seedComponentEstimateCalc); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['ComponentEstimateCalc'][$col])) 
            $displayColumns[$col] = $listViewDefs['ComponentEstimateCalc'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['ComponentEstimateCalc'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "ComponentEstimateCalc"); // builds the where clause from search field inputs
...
$lv->setup($seedComponentEstimateCalc, 'include/ListView/ListViewGeneric.tpl', $where, $params);
