...
$xtpl->assign('encoded_pressmachine_popup_request_data', $json->encode($popup_request_data));
...
$popup_request_data = array(
	...
	'field_to_name_array' => array(
		'id' => 'layoutid_popup',
		'name' => 'layoutname_popup',
		),
	);
$xtpl->assign('encoded_layout_popup_request_data', $json->encode($popup_request_data));
...
$xtpl->assign("JAVASCRIPT", get_set_focus_js().get_validate_record_js() . get_pressline_js() . $quicksearch_js);
...
$pressrows = $focus->getPresslineRows();
if(count($pressrows) == 0)
{
	$xtpl->assign("PRESSROWS",$focus->getPresslineRow(new Pressline(),0,true));
	$xtpl->parse("main.row1");		
}
for ($i=0;$i<count($pressrows);$i++) {
	    $fieldcount = count($pressrows[$i]);
		$xtpl->assign("PRESSROWS",$focus->getPresslineRow($pressrows[$i],$i, true));
		$xtpl->parse("main.row1");		
}
