...
if(file_exists('custom/modules/Press/metadata/listviewdefs.php')){
    require_once('custom/modules/Press/metadata/listviewdefs.php');  
}else{
    require_once('modules/Press/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Press');
...
$seedPress = new Press(); // seed bean
$searchForm = new SearchForm('Press', $seedPress); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Press'][$col])) 
            $displayColumns[$col] = $listViewDefs['Press'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Press'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Press"); // builds the where clause from search field inputs
...
$lv->setup($seedPress, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Press')));
$str = "<script>
