...
if(file_exists('custom/modules/Ratefilm/metadata/listviewdefs.php')){
    require_once('custom/modules/Ratefilm/metadata/listviewdefs.php');  
}else{
    require_once('modules/Ratefilm/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Ratefilm');
...
$seedRatefilm = new Ratefilm(); // seed bean
$searchForm = new SearchForm('Ratefilm', $seedRatefilm); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Ratefilm'][$col])) 
            $displayColumns[$col] = $listViewDefs['Ratefilm'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Ratefilm'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Ratefilm"); // builds the where clause from search field inputs
...
$lv->setup($seedRatefilm, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Ratefilm')));
$str = "<script>
