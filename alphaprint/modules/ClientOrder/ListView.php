...
if(file_exists('custom/modules/ClientOrder/metadata/listviewdefs.php')){
    require_once('custom/modules/ClientOrder/metadata/listviewdefs.php');  
}else{
    require_once('modules/ClientOrder/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'ClientOrder');
...
$seedClientOrder = new ClientOrder(); // seed bean
$searchForm = new SearchForm('ClientOrder', $seedClientOrder); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['ClientOrder'][$col])) 
            $displayColumns[$col] = $listViewDefs['ClientOrder'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['ClientOrder'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "ClientOrder"); // builds the where clause from search field inputs
...
$lv->setup($seedClientOrder, 'include/ListView/ListViewGeneric.tpl', $where, $params);
