...
if(file_exists('custom/modules/Ink/metadata/listviewdefs.php')){
    require_once('custom/modules/Ink/metadata/listviewdefs.php');  
}else{
    require_once('modules/Ink/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Ink');
...
$seedInk = new Ink(); // seed bean
$searchForm = new SearchForm('Ink', $seedInk); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Ink'][$col])) 
            $displayColumns[$col] = $listViewDefs['Ink'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Ink'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Ink"); // builds the where clause from search field inputs
...
$lv->setup($seedInk, 'include/ListView/ListViewGeneric.tpl', $where, $params);
