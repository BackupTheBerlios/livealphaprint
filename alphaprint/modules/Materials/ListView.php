...
if(file_exists('custom/modules/Materials/metadata/listviewdefs.php')){
    require_once('custom/modules/Materials/metadata/listviewdefs.php');  
}else{
    require_once('modules/Materials/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Materials');
...
$seedMaterial = new Material(); // seed bean
$searchForm = new SearchForm('Materials', $seedMaterial); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Materials'][$col])) 
            $displayColumns[$col] = $listViewDefs['Materials'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Materials'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Materials"); // builds the where clause from search field inputs
...
$lv->setup($seedMaterial, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Materials')));
$str = "<script>
