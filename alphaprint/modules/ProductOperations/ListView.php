...
if(file_exists('custom/modules/ProductOperations/metadata/listviewdefs.php')){
    require_once('custom/modules/ProductOperations/metadata/listviewdefs.php');  
}else{
    require_once('modules/ProductOperations/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'ProductOperations');
...
$seedProductOperation = new ProductOperation(); // seed bean
$searchForm = new SearchForm('ProductOperations', $seedProductOperation); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['ProductOperations'][$col])) 
            $displayColumns[$col] = $listViewDefs['ProductOperations'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['ProductOperations'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "ProductOperations"); // builds the where clause from search field inputs
...
$lv->setup($seedProductOperation, 'include/ListView/ListViewGeneric.tpl', $where, $params);
