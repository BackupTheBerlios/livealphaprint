...
function upgradeUWFiles($file) {
	...
	$cacheUploadUpgradesTemp = mk_temp_dir("{$sugar_config['upload_dir']}upgrades/temp");
	
	unzip($file, $cacheUploadUpgradesTemp);
	
	$allFiles = findAllFiles($cacheUploadUpgradesTemp, array());
	...
	foreach($allFiles as $k => $filepath) {
		if(strpos($filepath, "/UpgradeWizard/")) {
			$fileName = strstr($filepath, '/UpgradeWizard/');
			$destpath = getcwd()."/modules{$fileName}";
			
			logThis('updating UpgradeWizard code: '.$filepath);
			if(!copy($filepath, $destpath)) {
				logThis('*** ERROR: could not copy [ '.$filepath.' ] to destination');
			}
		}
	}
}
