...
if(file_exists('custom/modules/Presspricelist/metadata/listviewdefs.php')){
    require_once('custom/modules/Presspricelist/metadata/listviewdefs.php');  
}else{
    require_once('modules/Presspricelist/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Presspricelist');
...
$seedPresspricelist = new Presspricelist(); // seed bean
$searchForm = new SearchForm('Presspricelist', $seedPresspricelist); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Presspricelist'][$col])) 
            $displayColumns[$col] = $listViewDefs['Presspricelist'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['Presspricelist'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Presspricelist"); // builds the where clause from search field inputs
...
$lv->setup($seedPresspricelist, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Presspricelist')));
$str = "<script>
