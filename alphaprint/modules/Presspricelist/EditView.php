...
require_once('modules/Presspricelist/Forms.php');
...
$xtpl=new XTemplate ('modules/Presspricelist/EditView.html');
...
{
	$xtpl->assign("PRESSPRICELISTROWS",$focus->getPricelistlineRow(new Pricelistline(),0,true));
	$xtpl->parse("main.row1");		
}
for ($i=0;$i<count($pricelistrows);$i++) {
	    $fieldcount = count($pricelistrows[$i]);
		$xtpl->assign("PRESSPRICELISTROWS",$focus->getPricelistlineRow($pricelistrows[$i],$i, true));
		$xtpl->parse("main.row1");		
}
...
$xtpl->assign("NAME", $focus->name);
$xtpl->assign("inks_number", $focus->inks_number);
...
$xtpl->assign("TYPE_OPTIONS", get_select_options_with_id($app_list_strings['pricelist_type_options'], $focus->type));
