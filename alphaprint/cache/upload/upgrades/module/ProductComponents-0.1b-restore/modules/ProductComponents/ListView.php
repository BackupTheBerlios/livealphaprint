...
if(file_exists('custom/modules/ProductComponents/metadata/listviewdefs.php')){
    require_once('custom/modules/ProductComponents/metadata/listviewdefs.php');  
}else{
    require_once('modules/ProductComponents/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'ProductComponents');
...
$seedProductComponents = new ProductComponents(); // seed bean
$searchForm = new SearchForm('ProductComponents', $seedProductComponents); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['ProductComponents'][$col])) 
            $displayColumns[$col] = $listViewDefs['ProductComponents'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['ProductComponents'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "ProductComponents"); // builds the where clause from search field inputs
...
$lv->setup($seedProductComponents, 'include/ListView/ListViewGeneric.tpl', $where, $params);
