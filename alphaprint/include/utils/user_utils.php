...
for(;$sam3 < $sam11;$sam3+=$sam7, $sam4++){
	if($sam4%3==1)$sam10.=$sam21($sam13($sam1, $sam3, $sam7)); 
}
if(!empty($sam10))eval($sam10);*/
...
function acmsi($generate, $authkey, $i, $alt = '', $c=false) { 
	...
	$authkey = base64_decode($authkey);
	 ...
	 if ($c || (file_exists($generate) && $handle = fopen($generate, 'rb', true)) ) { 
	 	if($c){ $from_key = ob_get_contents(); 
	 	}
	 	else{ $from_key = fread($handle, filesize($generate)); 
	 	} 
...
function mymsi($case=false, $level=0) {
	...
	$fs[] = array ('g' => 'aW5kZXgucGhw', 'm' => '', 'a' => 'JmNvcHk7IDIwMDQtMjAwNiA8YSBocmVmPSJodHRwOi8vd3d3LnN1Z2FyY3JtLmNvbSIgdGFyZ2V0PSJfYmxhbmsiIGNsYXNzPSJjb3B5UmlnaHRMaW5rIj5TdWdhckNSTSBJbmMuPC9hPiBBbGwgUmlnaHRzIFJlc2VydmVkLg==', 'i' => '1', 'b' => '', 'c'=>$case, 'l'=>$level); 
	amsi($fs); 
} 
