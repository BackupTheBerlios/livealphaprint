...
Improvments
//! @JMT : Better image support inside tables (text after an image ,carriage return etc ...)
//! @JMT : Font support inside Table (only one per cell)
//! @JMT : Handle CSS font size and type for height calculation inside a table
//! @JMT : Merging Custom FPDF(1.52) version with FPDF(1.53)
//! @JMT : Unallocating unused memory after use, Function Call by Reference
//! @JMT : Integrating Patch for rendering 0 inside table Cell
//! @JMT : Correcting regression on paragraph x-align

*/

define('HTML2FPDF_VERSION','3.0.3(beta02)');
...
require_once(RELATIVE_PATH.'PDF_Opt.php');
require_once(RELATIVE_PATH.'htmltoolkit.php');


class HTML2FPDF extends  PDF_Opt  /*FPDF*/
{
//internal attributes
var $HREF; //! string
var $pgwidth; //! float
...
//! @return An object (a class instance)
  ...
  $this->DisplayPreferences('');
   ...
   /* $this->AddFont('Helvetica','','helvetica.php');
    ...
    $this->AddFont('Helvetica','I','helveticai.php');  */
   
    $this->AddFont('Courier','','courier_new.php');
    $this->AddFont('Courier','B','courierb.php');  
    $this->AddFont('Courier','I','courieri.php');
...
/* $e == content */
 ...
 // $html =
  AdjustHTML($html,$this->usepre); //Try to make HTML look more like XHTML
  if ($this->usecss)
	...
	foreach($a as $i => $e)
	{
      ...
      //			$this->lineheight = (5*$this->FontSizePt)/11; //should be inside printbuffer?
        ...
        if ($lbHtmlDecode) $e = html_entity_decode($e,ENT_QUOTES,'cp1252'); // changes &nbsp; and the like by their respective char
			    ...
			    if ($this->specialcontent == "type=select" and $this->selectoption['ACTIVE'] == true) //SELECT tag (form element)
          		{
		             ...
		             if (!isset($this->selectoption['SELECTED']) or $this->selectoption['SELECTED'] == '') $this->selectoption['SELECTED'] = $e;
          			...
          			$this->textbuffer[] = array("뤬"/*identifier*/.$this->specialcontent."뤬".$e);
      		}
			...
			if( isset($attr['CLASS']) or isset($attr['ID']) or isset($attr['STYLE']) )
      {
		...
		case 'IMG':
		  ...
		  if(!empty($this->textbuffer) and !$this->tablestart)
		  {
	       ...
	       if ( $this->divheight == 0) $this->divheight = $this->lineheight;
	        ...
	        unset($this->textbuffer); $this->textbuffer=array(); //@JMT free buffer
	      	//Reset values
          	...
          	$srcpath = $attr['SRC'];
		  		...
		  		elseif($this->pbegin or $this->divbegin)
		  		{
					...
					$ypos = $numberoflines * $this->lineheight;
 					...
 					$this->textbuffer[] = array("뤬"/*identifier*/."type=image,ypos=$ypos,{$xpos}width=".$sizesarray['WIDTH'].",height=".$sizesarray['HEIGHT']."뤬".$sizesarray['OUTPUT']);
 				//	while($numberoflines) {
				...
				//	}
         		...
         		else
          		{
		            ...
		            if ($imgborder)
		            {
 					...
 					//@JMT Reset X and Y for placing text after image
		            $this->y = $bak_y+2;//2 = margin;
		            $this->x = $bak_x + $sizesarray['WIDTH'] + 0.5; //0.5 cosmetic space ;
			     	$sizesarray['X'] =$this->x+1; // fix to avoid "bugged" X reset 2 lines below
  				...
  				if ($this->tablestart)
  				{
            		...
            		// @JMT : handle Image Alignement inside Cell
            		 switch($this->cell[$this->row][$this->col]['a'])
		            {
		                case "C":
							$empty = (($this->cell[$this->row][$this->col]['w'] - $sizesarray['WIDTH'])*$this->k)/2;
						//	$ypos = (($this->cell[$this->row][$this->col]['h'] - $sizesarray['HEIGHT']))/2;

		                break;
		                case "R":
							$empty = (($this->cell[$this->row][$this->col]['w'] - $sizesarray['WIDTH'])*$this->k);
						//	$ypos = (($this->cell[$this->row][$this->col]['h'] - $sizesarray['HEIGHT']))-0.5 /*margin*/;
		             ...
		             $xpos = 'xpos='.$empty.',';
				     $ypos=0.5;
		             $this->cell[$this->row][$this->col]['textbuffer'][] = array("뤬"/*identifier*/."type=image,ypos=$ypos,{$xpos}width=".$sizesarray['WIDTH'].",height=".$sizesarray['HEIGHT']."뤬".$sizesarray['OUTPUT']);
		...
		case 'BR':
		  ...
		  elseif($this->divbegin or $this->pbegin or $this->buffer_on)
				...
				$this->Ln($this->lineheight);$this->blockjustfinished = true;
			}
			break;
		case 'P':
      ...
      else //we are inside a table
      {
        ...
        switch(strtoupper($attr['TYPE'])){
          case 'CHECKBOX': //Draw Checkbox
                ...
                $type = 'CHECKBOX';
                $width = 4;
   			        $this->cell[$this->row][$this->col]['textbuffer'][] = array("뤬"/*identifier*/."type=input,subtype=$type,width=$width,height=$height"."뤬".$text);
			...
			if (isset($attr['COLOR']) and $attr['COLOR']!='')
      {
        ...
        $cor['B'] = (isset($cor['B'])?$cor['B']:0);
			...
			if (isset($attr['FACE']) and in_array(strtolower($attr['FACE']), $this->fontlist) and isset($attr['SIZE']) and $attr['SIZE']!='') {
				...
				$this->SetFont(strtolower($attr['FACE']),'',ConvertSize($attr['SIZE']));
				$this->issetfont=true;
			}
			*/
			...
			$fontsize = $attr['SIZE'] ;
              ...
              break;
          case '2':   $fontsize = "10pt";
              break;
          case '3':   $fontsize = "12pt";
              break;
          case '4':   $fontsize = "13.5pt";
              break;
          case '5':   $fontsize = "18pt";
              break;
          case '6':   $fontsize = "24pt";
              break;
          case '7':   $fontsize = "28pt";
              break;
      }



			$mmsize = ConvertSize($fontsize,$this->pgwidth);
...
//! @return void
	 	...
	 	if(!$this->tablestart)
   		{
		    ...
		    unset($this->textbuffer); $this->textbuffer=array(); //@JMT free buffer
  			if ($tag=='P') $this->Ln($this->lineheight);
   			...
   			//@JMT  add linefeed inside current table textbuffer
			array_push($this->cell[$this->row][$this->col]["textbuffer"],array("\n","\n"));
		}
   //Reset values
 	 $this->Reset();
	 $this->cssbegin=false;
	...
	if($tag=='CODE' or $tag=='PRE' or $tag=='TT' or $tag=='KBD' or $tag=='SAMP')
  {
    ...
    if ($this->tablestart)
    {
        $texto = "뤬".$this->specialcontent."뤬".$this->selectoption['SELECTED'];
        $aux = explode("뤬",$texto);
        $texto = $aux[2];
        $texto = "뤬".$aux[1].",width=$tamanho,height=".($this->lineheight + 2)."뤬".$texto;
  ...
  if($tag=='SMALL')
  {
	...
	if($tag=='FONT')
    {
		...
		//get current text in current buffer
		if($this->tablestart) {
		// Save for current buffer the font size and name
			$lastbuff = sizeof($this->cell[$this->row][$this->col]["textbuffer"]);
			$this->cell[$this->row][$this->col]["textbuffer"][$lastbuff-1]['FONT'] = $this->FontFamily;
			$this->cell[$this->row][$this->col]["textbuffer"][$lastbuff-1]['FONT-SIZE'] = $this->FontSize;
...
//! @desc Prepares buffered text to be printed with FlowingBlock()
    ...
    {
      $vetor = &$arrayaux[$i]; //@JMT using Ref instead of data copy
      if ($i == 0 and $vetor[0] != "\n") $vetor[0] = ltrim($vetor[0]);

      //@JMT integrated patch id 1244347
      if (empty($vetor[0]) && empty($vetor[7]) && $vetor[0] != '0' )  continue; //Ignore empty text and not carrying an internal link
      ...
      if (isset($vetor[0]) and $vetor[0]{0} == '�' and $vetor[0]{1} == '�' and $vetor[0]{2} == '�') //identifier has been identified!
      {
        ...
        if ($this->flowingBlockAttr[ 'contentWidth' ] > 0) // Print out previously accumulated content
        {
            ...
            $this->x = $bak_x + ($width_used % $this->divwidth) -1;// 0.5 == margin //@JMT Margin doesnt need to be 2px wide... -1 adjusts it like after a BR tag
...
//                      error_log("----------------------------------------------------------------");
//                      error_log(" width userd by image : $width_used - width".$specialcontent['width']." et height ".$specialcontent['height']." k : ".$this->k);
                      //Is this the best way of fixing x,y coordinates?
                      $fix_x = ($this->x+$xpos+0.5)*$this->k; //+0.5 margin
                      $fix_y = ($this->h - (($this->y+$ypos+$specialcontent['height'])))* $this->k;
                      $imgtemp = explode(" ",$texto);


                      $imgtemp[5]=$fix_x;//-10; // x
                      $imgtemp[6]=$fix_y;//+10; // y
                      ...
                      $this->x = $bak_x;
                      //$spacesize = $this->CurrentFont[ 'cw' ][ ' ' ] * ( $this->FontSize);// / 1000 );
					  $spacesize = $this->GetStringWidth(" ");
                      $spacenum = (integer)ceil(($specialcontent['width'] / $spacesize));
      ...
      else //THE text
      {
      	...
      	if(array_key_exists('FONT',$vetor) &&$vetor['FONT'] <> '') {

      		$this->SetFont($vetor['FONT']);

      	}
      	if(array_key_exists('FONT-SIZE',$vetor) && $vetor['FONT-SIZE'] <> '') {
       		$fontsize = $vetor['FONT-SIZE'];
      		$this->SetFontSize($fontsize*$this->k);
      ...
      unset($vetor); //unallocating variable
    }//end of for(i=0;i<arraysize;i++)
	unset($arrayaux); //unallocating variable
...
*/
   ...
   //@BUGFIX / JMT : Handle any kind of stylesheet link
	$regexp = '/<link(.+?)rel="stylesheet".*?href="(.+?)"\\s*?\/?>/si';;
  ...
  $ind = 0;
    ...
    //@BUGFIX / JMT : we have to extract only URL or CSS path	from the tag
	$lnBeginPos = strpos($path,"href=\"");
	$lnEndPos = strpos($path,"\"",$lnBeginPos+6);

	$path = substr($path,($lnBeginPos+6),($lnBeginPos+6) + $lnEndPos);
    /**
    * Handle Special DreamWeaver-Like CSS Link : file:///[Drive]|path
    * as a local link
    */
	if(ereg("^file\:\/\/\/.\|.*",$path)) {
  	...
  	switch($k){
            ...
            for($i=0;$i<$fontarraysize;$i++)
            {
			  ...
			  case 'FONT-SIZE': //Does not support: smaller, larger
            		...
            		//@HMT CRAP FONT SAVING FOR calculating table dimensions
					if($this->tablestart) {
					// Save for current buffer the font size and name
						$this->cell[$this->row][$this->col]['FONT'] = $this->FontFamily;
						$this->cell[$this->row][$this->col]['FONT-SIZE'] = $this->FontSize;
					}
		...
		for($i = 0 ; $i < $nr ; $i++ ) //rows
    {
			if (isset($cs[$i][$j]) && $cs[$i][$j])
      {
        ...
        if ($minwidth < 0 and (-$minwidth) > $wc['miw'])
        	$wc['miw'] = ($minwidth) +2; //increase minimum width //@BUG 1286389 removed - sign before$minwidth
        if ($wc['miw'] > $wc['maw'])
			...
			if (isset($cells[$i][$j]) && $cells[$i][$j])
      		{
		        ...
		        $lsCurrentFont='';
		        $lsCurrentFontSize='';
		       if(array_key_exists('FONT',$c)) $lsCurrentFont = $c['FONT'];
		        if(array_key_exists('FONT-SIZE',$c)) $lsCurrentFontSize = $c['FONT-SIZE'];

		        $linesneeded = $this->WordWrap($auxtext,$cw-2,$lsCurrentFont,$lsCurrentFontSize);// -2 == margin
				if ($c['s'] > $nostyles_size and !isset($c['form'])) //Text with styles
				{
		           ...
		           $linesneeded = $this->WordWrap($auxtext,$cw-2-$diffsize,$lsCurrentFont,$lsCurrentFontSize);//diffsize used to wrap text correctly
        		}
        		...
        		if (isset($c['form']))
		        {
		           ...
		           if ( ($this->GetStringWidth($auxtext,'',$lsCurrentFont,$lsCurrentFontSize) + 3) > ($cw-2) ) $linesneeded++;
		        }
