...
class json_config {
	...
	function getAppMetaJSON($scheduler = false) {
        ...
        // Bug 8874: Jenny - This case handles the situation where the site config is set to https://
        // but the user hits the url as http://.  
        if (!empty ($_SERVER['HTTP_REFERER'])) {
            $http_base = split(':', $_SERVER['HTTP_REFERER'], 2);
            $sugar_config['site_url'] = preg_replace('/^http(s)?\:\/\/[^\/]+/',$http_base[0]."://".$_SERVER['HTTP_HOST'],$sugar_config['site_url']);
        }
        else
