...
//Peter Peshev
function get_select_options_with_id_detailview ($option_list, $selected_key) {
	return get_select_options_with_id_separate_key_detailview($option_list, $option_list, $selected_key);
}
...
// Peter Pehev
function get_select_options_with_id_separate_key_detailview ($label_list, $key_list, $selected_key) {
	global $app_strings;
	$select_options = "";
...
function parse_list_modules(&$listArray)
{
	...
	foreach($listArray as $optionName => $optionVal)
	{
		...
		/*if(array_key_exists('Products', $modListHeader))
		{
			...
			//Peter Peshev
			$returnArray['ProductComponents'] = $listArray['ProductComponents'];
		}  */
...
function pre_login_check(){
	global $action, $login_error, $app_strings;
	if(!empty($action)&& $action == 'Login'){
		...
		if(!empty($login_error) && $login_error != $app_strings['LOGIN_LOGO_ERROR'] ){
...
function convert_module_to_singular($module_array){
	...
	foreach($module_array as $key => $value){
		...
		if($key=="productcomponents"){
				$module_array['ProductComponents'] = "Product Components";
				unset($module_array[$key]);
		}
	}
