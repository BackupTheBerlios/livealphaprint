...
class SugarPHPMailer extends PHPMailer {
	...
	function prepForOutbound() {
		...
		if($this->preppedForOutbound == false) {
			...
			// body text
			$this->Body			= $locale->translateCharset(trim($this->Body), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
			$this->AltBody			= $locale->translateCharset(trim($this->AltBody), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));

			// Headers /////////////////////////////////
			$this->Subject			= $locale->translateCharsetMIME(trim($this->Subject), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
			$this->From			= $locale->translateCharsetMIME(trim($this->From), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
			$this->FromName		= $locale->translateCharsetMIME(trim($this->FromName), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
			$this->Sender			= $locale->translateCharsetMIME(trim($this->Sender), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
			foreach($this->ReplyTo as $k => $v) {
				$this->ReplyTo[$k][0] = $locale->translateCharsetMIME(trim($v[0]), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
				$this->ReplyTo[$k][1] = $locale->translateCharsetMIME(trim($v[1]), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
			}
			
			// TO: fields
			foreach($this->to as $k => $toArr) {
				$this->to[$k][0]	= $locale->translateCharsetMIME(trim($toArr[0]), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
				$this->to[$k][1]	= $locale->translateCharsetMIME(trim($toArr[1]), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
			}
			// CC: fields
			foreach($this->cc as $k => $ccAddr) {
				$this->cc[$k][0]	= $locale->translateCharsetMIME(trim($ccAddr[0]), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
				$this->cc[$k][1]	= $locale->translateCharsetMIME(trim($ccAddr[1]), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
			}
			// BCC: fields
			foreach($this->bcc as $k => $bccAddr) {
				$this->bcc[$k][0]	= $locale->translateCharsetMIME(trim($bccAddr[0]), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
				$this->bcc[$k][1]	= $locale->translateCharsetMIME(trim($bccAddr[1]), 'UTF-8', $locale->getPrecedentPreference('default_email_charset'));
			}
		}
